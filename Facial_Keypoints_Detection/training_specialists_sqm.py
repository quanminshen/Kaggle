...
def train_specialist1(spec_setting):
    """
    Trains a single specialist as per its settings.
    ...
    with graph.as_default():
        ...
        summary_writer = tf.summary.FileWriter('models/logs', sess.graph)
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % 100 == 0:
                    print('Step {}: loss = {:.2f} time: {}'.format(step, loss_value, get_time_hhmmss(spec_start)))
...
for spec_setting in SPECIALIST_SETTINGS:
    train_specialist1(spec_setting)
